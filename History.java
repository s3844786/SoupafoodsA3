public class History {
	...
	public History() {
		...
		this.Data = new String[NumProductCategories];
	...
	public void SearchProductFromEntry() {
		...
		while(i < this.CurrentNumProductCategories) {
			if(this.Data[i].contains(searchTargetName)) {
				String rowData = this.Data[i] + "\t" + this.ProductNames[i] + "\t" + this.ProductNums[i] + "\t"
						+ this.Prices[i];
	...
	public void addProduct(String data, String names, int numbers, double prices) {
		int i = getProductIndexByName(data);
		if(i<0) {
			this.Data[this.CurrentNumProductCategories] = data;
	...
	public int getProductIndexByName(String search) {
		...
		while ((FindSearch < this.CurrentNumProductCategories)
				&& (this.Data[FindSearch].compareTo(search) != 0)) {
			FindSearch++;
		}
	...
	public void Edit() {
		...
		if (edit != null) {
			...
			this.Data[i] = this.gt.getInputString("Please enter the data:");
			...
			this.gt.addRowToTable(0, this.Data[i] + "\t" + this.ProductNames[i] + "\t" + this.ProductNums[i] + "\t" + this.Prices[i]);
			Refresh();
	...
	public void InventoryChanges() {
		...
		String data = fieldsOfProducts[0];
		...
		this.gt.addRowToTable(0, data + "\t" + name + "\t" + number + "\t" + price);
